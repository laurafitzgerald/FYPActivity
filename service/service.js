...
var activitytable = '{ "columns": { "name": "text","timestamp": "text", "type": "text" ,"time": "decimal", "distance": "decimal", "speed": "decimal", "username": "text"  },"primary": ["timestamp","username"],"indexes": ["username"]}';
...
nc.subscribe('activity.delete', function(msg,reply,sub){
	...
	var username = "username='" +msg.username +"'";
	var timestamp = "timestamp='" + msg.timestamp +"'";
	var filter  = { "filters" : [ username , timestamp]};
	nc.request('data.delete.activity', JSON.stringify(filter) , function(response){
		nc.publish(reply, response);
	});
